...
class DataAnalyzer:
    ...
    def _try_parse_datetime(self, series):
        """Parse likely datetime text without noisy pandas format warnings."""
    ...
    def _analyze_columns(self):
        ...
        for col in self.df.columns:
            ...
            # 分类型分析
                ...
                # 尝试转换为时间序列
                parsed_datetime = self._try_parse_datetime(self.df[col])
                valid_ratio = float(parsed_datetime.notna().mean())
                if valid_ratio >= 0.8 and parsed_datetime.nunique(dropna=True) >= 2:
                    self.df[col] = parsed_datetime
                    info['type'] = 'datetime'
                    info['min'] = parsed_datetime.min()
                    info['max'] = parsed_datetime.max()
    ...
    def _generate_suggestions(self):
        ...
        if len(numeric_cols) >= 2 and (datetime_cols or categorical_cols):
            ...
            if len(plots_for_dashboard) >= 2:
                dashboard_rows = (len(plots_for_dashboard) + 1) // 2
                suggestions.append({
                    'id': 6,
                    'name': '综合仪表盘',
                    'type': 'dashboard',
                    'layout': f'{dashboard_rows}x2',
