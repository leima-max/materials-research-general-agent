...
If dependencies are missing, install them into this skill only:

```bash
python scripts/install_dependencies.py
```

The scripts prefer `vendor/site-packages` inside this skill when present, so
portable installs do not need to modify global Python packages.
