...
| `expGraph` (LabTalk) | Produces no file when `path` is a full file path | Use `path` as output directory and `filename` as file stem |
...
Our MCP uses **Origin expGraph with file verification**. This means:
- Export writes directly to the requested folder
- The tool verifies that the exported file exists and is non-trivial in size
- If export fails, inspect the returned message before retrying

---

## Font Size Guide

Use these sizes for readable publication output:
