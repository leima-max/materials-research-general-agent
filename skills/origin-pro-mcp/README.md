...
| `expGraph` path misuse produces no files | Use `path` as the output directory and `filename` as the file stem |
...
| Export returns empty | Verify `expGraph` receives a directory in `path`, then check the returned file-size message |
