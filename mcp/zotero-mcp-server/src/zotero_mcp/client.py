...
def _read_api_key() -> str | None:
    """Read a Zotero API key from the environment or a configured key file."""
    api_key = os.getenv("ZOTERO_API_KEY")
    if api_key:
        return api_key.strip()

    key_file = os.getenv("ZOTERO_API_KEY_FILE")
    if not key_file:
        return None

    try:
        path = Path(os.path.expandvars(key_file)).expanduser()
...
def get_zotero_client() -> zotero.Zotero:
    ...
    if local:
        # Zotero's desktop local API uses user ID 0 for the active personal
        # library, while hybrid web writes need the real zotero.org user ID.
        library_id = override.get("library_id") or os.getenv("ZOTERO_LOCAL_LIBRARY_ID") or "0"
    else:
...
def get_web_zotero_client() -> zotero.Zotero | None:
    ...
    """
    library_id = os.getenv("ZOTERO_WEB_LIBRARY_ID") or os.getenv("ZOTERO_LIBRARY_ID")
