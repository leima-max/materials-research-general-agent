...
def _try_attach_oa_pdf(write_zot, item_key, doi, ctx, crossref_metadata=None,
    ...
    if found_urls:
        ...
        source_name, url_info = found_urls[0]  # Best URL found
        if attach_mode == "auto":
            ctx.info(
                "Direct PDF download failed, falling back to a linked PDF URL "
                f"from {source_name}"
            )
            if _attach_pdf_linked_url(write_zot, url_info, item_key, ctx):
                return (
                    f"PDF linked (source: {source_name}; direct download failed)"
                )
        return (
