                ...
                if not _check_cascade_timeout() and not items:
                    try:
                        ...
                        if config_path.exists():
                            ...
                            t0 = _time.monotonic()
                            remaining = max(10, int(CASCADE_TIMEOUT - (_time.monotonic() - _cascade_start)))
                            sem_response = _safe_semantic.run_semantic_operation(
                                "search",
                            ...
                            _search_logger.debug(f"[CASCADE] semantic worker: {_time.monotonic() - t0:.2f}s")
                            if not sem_response.get("ok"):
                                raise RuntimeError(_safe_semantic.semantic_error_message(sem_response))
                            sem_results = sem_response.get("result") or {}
    ...
    *,
    ...
    try:
        ...
        if not response.get("ok"):
            return (
                "Semantic search backend is unavailable, but the MCP server stayed alive.\n\n"
                f"Reason: {_safe_semantic.semantic_error_message(response)}\n\n"
                "Use zotero_search_items for keyword search, or rebuild the semantic database "
                "with zotero_update_search_database after the local embedding stack is healthy."
            )
    ...
    try:
        ...
        if not response.get("ok"):
            return (
                ...
                "**Error:** Semantic database update failed in the isolated worker.\n\n"
                f"**Reason:** {_safe_semantic.semantic_error_message(response)}"
            )
