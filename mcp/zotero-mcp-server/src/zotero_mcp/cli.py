...
def _configure_stdio() -> None:
    """Use UTF-8 console streams on Windows to avoid GBK encode crashes."""
...
def setup_zotero_environment():
    ...
    if not (os.environ.get("ZOTERO_API_KEY") or os.environ.get("ZOTERO_API_KEY_FILE")):
        fallback_env_vars = {
            "ZOTERO_LOCAL": "true",
            "ZOTERO_LIBRARY_ID": "0",
        }
...
def main():
    ...
    # Kimi Code smoke test command
    kimi_code_parser = subparsers.add_parser("kimi-code-smoke", help="Test Kimi Code API chat access")
    kimi_code_parser.add_argument("--prompt", default="Reply with exactly: OK",
                                  help="Prompt to send to Kimi Code (default: minimal OK check)")
    kimi_code_parser.add_argument("--base-url",
                                  help="Override Kimi Code OpenAI-compatible base URL")
    kimi_code_parser.add_argument("--model",
                                  help="Override Kimi Code model ID (default: kimi-for-coding)")
    kimi_code_parser.add_argument("--protocol", choices=["anthropic", "openai", "auto"],
                                  help="Kimi Code protocol (default: anthropic)")
    kimi_code_parser.add_argument("--max-tokens", type=int, default=16,
                                  help="Max tokens for the smoke response")
    ...
    if args.command == "version":
    ...
    elif args.command == "kimi-code-smoke":
        from zotero_mcp.kimi_code_client import KimiCodeClient, load_kimi_code_config, redact_secrets

        try:
            config = load_kimi_code_config(base_url=args.base_url, model=args.model, protocol=args.protocol)
            print("=== Kimi Code API Smoke Test ===")
            print(f"Config: {json.dumps(config.redacted, ensure_ascii=False, separators=(',', ':'))}")
            client = KimiCodeClient(config)
            result = client.chat(args.prompt, max_tokens=args.max_tokens, temperature=0.0)
            print("Status: OK")
            print(f"Model: {result.get('model') or config.model}")
            print(f"Protocol: {result.get('protocol') or config.protocol}")
            if result.get("usage"):
                print(f"Usage: {json.dumps(result['usage'], ensure_ascii=False, separators=(',', ':'))}")
            content = (result.get("content") or "").strip()
            if content:
                print(f"Response: {redact_secrets(content)}")
        except Exception as e:
            print("=== Kimi Code API Smoke Test ===")
            print("Status: ERROR")
            print(f"Error: {redact_secrets(f'{type(e).__name__}: {e}')}")
            sys.exit(1)
        sys.exit(0)
        ...
        if config_path.exists():
            try:
                ...
                response = _safe_semantic.run_semantic_operation("status", config_path=config_path)
                if not response.get("ok"):
                    raise RuntimeError(_safe_semantic.semantic_error_message(response))
                ...
                if collection_info.get("embedding_signature_mismatch"):
                    print("  Warning: embedding config changed; run update-db --force-rebuild before semantic search")
