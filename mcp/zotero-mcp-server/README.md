...
### Word/Docx Zotero Dynamic Fields
- When inserting citations into Word or `.docx`, the default output must be Zotero dynamic fields, not plain text.
- Citation fields must be Word fields containing `ADDIN ZOTERO_ITEM CSL_CITATION`; bibliographies must be Zotero bibliography fields containing `ADDIN ZOTERO_BIBL`.
- Do not treat manually typed bracketed numbers such as `[1]` or an appended bibliography as a successful Zotero Word insertion unless the user explicitly requested static references.
- Preferred tools are `zotero_word_insert_citations` for citation fields, `zotero_word_plugin_command` with `addBibliography` or `refresh` for bibliography/refresh actions, and `zotero_word_refresh_and_verify` for verification.
- Before delivery, verify dynamic fields via Word COM field counts or by inspecting `word/document.xml` for `ADDIN ZOTERO_ITEM`, `CSL_CITATION`, and `ADDIN ZOTERO_BIBL`.
...
### Word/Docx Citation Tools
- `zotero_word_insert_citations`: Insert Zotero-compatible dynamic citation fields; this is the default route for Word/Docx citation insertion.
- `zotero_word_batch_auto_cite_document`: Batch-plan and insert Zotero dynamic citation fields across a Word document.
- `zotero_word_plugin_command`: Invoke Zotero Word plugin commands such as `addBibliography`, `refresh`, and `setDocPrefs`.
- `zotero_word_refresh_and_verify`: Refresh and verify Zotero citation fields and bibliography fields.
